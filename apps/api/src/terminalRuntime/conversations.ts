...
export type ConversationTranscriptEventPayload =
  | Omit<SessionStartTranscriptEvent, "eventId" | "sessionId" | "armId">
  | Omit<InputSubmitTranscriptEvent, "eventId" | "sessionId" | "armId">
  | Omit<OutputChunkTranscriptEvent, "eventId" | "sessionId" | "armId">
  | Omit<StateChangeTranscriptEvent, "eventId" | "sessionId" | "armId">
  | Omit<SessionEndTranscriptEvent, "eventId" | "sessionId" | "armId">;
...
const parseTranscriptEvent = (value: unknown): ConversationTranscriptEvent | null => {
  ...
  const armId = asString(value.armId);
  const timestamp = asString(value.timestamp);

  if (!eventType || !eventId || !sessionId || !armId || !timestamp) {
    return null;
  }
