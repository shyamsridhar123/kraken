...
class FakeGitClient implements GitClient {
  ...
  createPullRequest({
    ...
    const pullRequest = {
      number: nextNumber,
      url: `https://github.com/kraken-project/kraken/pull/${nextNumber}`,
      title,
      baseRef,
      headRef,
      state: "OPEN" as const,
...
describe("createApiServer", () => {
  ...
  const startServer = async (options: Partial<Parameters<typeof createApiServer>[0]> = {}) => {
    const workspaceCwd =
      ...
      (() => {
        const directory = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
  ...
  const waitForRegistryDocument = async <TDocument>(
    ...
    const registryPath = join(workspaceCwd, ".kraken", "state", "arms.json");
  ...
  const writeConversationTranscript = (
    ...
    const transcriptDirectory = join(workspaceCwd, ".kraken", "state", "transcripts");
  ...
  it("returns session summaries for GET /api/conversations", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
  ...
  it("POST /api/hooks/user-prompt-submit auto-renames generated default terminal names", async () => {
    ...
    const hookResponse = await fetch(
      `${baseUrl}/api/hooks/user-prompt-submit?kraken_session=terminal-1`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    await expect(listResponse.json()).resolves.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          armName: "Investigate flaky CI failures",
        }),
      ]),
    );
  });
  ...
  it("infers generated terminal names from older registry entries", async () => {
    ...
    temporaryDirectories.push(workspaceCwd);
    const registryPath = join(workspaceCwd, ".kraken", "state", "arms.json");
    mkdirSync(join(workspaceCwd, ".kraken", "state"), { recursive: true });
    writeFileSync(
      registryPath,
      `${JSON.stringify(
        {
  ...
  it("reports file-backed workspace setup status and updates it through setup actions", async () => {
    ...
    expect(existsSync(join(workspaceCwd, ".kraken"))).toBe(false);
    ...
    expect(initialPayload.hasAnyArms).toBe(false);
    expect(initialPayload.steps).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: "create-arms", complete: false }),
      ]),
    );
    ...
    expect(existsSync(join(workspaceCwd, ".kraken", "project.json"))).toBe(true);
    expect(existsSync(join(workspaceCwd, ".kraken", "arms"))).toBe(true);
    expect(existsSync(join(workspaceCwd, ".kraken", "worktrees"))).toBe(true);
    ...
    expect(readFileSync(join(workspaceCwd, ".gitignore"), "utf8")).toContain(".kraken");

    const createArmResponse = await fetch(`${baseUrl}/api/commandDeck/arms`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    expect(createArmResponse.status).toBe(201);
    ...
    expect(finalPayload.hasAnyArms).toBe(true);
    expect(finalPayload.armCount).toBe(1);
    expect(finalPayload.steps).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: "create-arms", complete: true }),
      ]),
    );
  });

  it("returns 413 when create arm body exceeds size limit", async () => {
    const baseUrl = await startServer();
  ...
  it("lists Claude skills from the project skills folder", async () => {
    ...
    const baseUrl = await startServer({ workspaceCwd });
    const response = await fetch(`${baseUrl}/api/commandDeck/skills`, {
      headers: { Accept: "application/json" },
    });
  ...
  it("creates arms with suggested skills and appends the managed context block", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const baseUrl = await startServer({ workspaceCwd });

    const response = await fetch(`${baseUrl}/api/commandDeck/arms`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const context = readFileSync(
      join(workspaceCwd, ".kraken", "arms", "docs", "CONTEXT.md"),
      "utf8",
    );
    ...
    const listResponse = await fetch(`${baseUrl}/api/commandDeck/arms`, {
      headers: { Accept: "application/json" },
    });
    expect(listResponse.status).toBe(200);
    await expect(listResponse.json()).resolves.toEqual(
  ...
  it("updates arm suggested skills and removes the managed context block when cleared", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const baseUrl = await startServer({ workspaceCwd });

    const createResponse = await fetch(`${baseUrl}/api/commandDeck/arms`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const updateResponse = await fetch(`${baseUrl}/api/commandDeck/arms/docs/skills`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    const contextPath = join(workspaceCwd, ".kraken", "arms", "docs", "CONTEXT.md");
    expect(readFileSync(contextPath, "utf8")).toContain("- `code-review-specialist`");

    const clearResponse = await fetch(`${baseUrl}/api/commandDeck/arms/docs/skills`, {
      method: "PATCH",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    expect(readFileSync(contextPath, "utf8")).not.toContain("kraken:suggested-skills:start");
  });

  it("returns 400 for unsupported arm completion sound values", async () => {
    const baseUrl = await startServer();
  ...
  it("creates new arms with unique incremental ids", async () => {
    const baseUrl = await startServer();
  ...
  it("reuses the minimum available arm number after deletions", async () => {
    const baseUrl = await startServer();
  ...
  it("ignores stale persisted nextArmNumber values and starts from the minimum available id", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const registryPath = join(workspaceCwd, ".kraken", "state", "arms.json");
    mkdirSync(join(workspaceCwd, ".kraken", "state"), { recursive: true });
  ...
  it("skips arm ids that already have an existing worktree directory", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    mkdirSync(join(workspaceCwd, ".kraken", "worktrees", "terminal-1"), {
      recursive: true,
    });
  ...
  it("persists arm metadata without runtime bootstrap flags", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const baseUrl = await startServer({
      workspaceCwd,
  ...
  it("injects a default arm context prompt for arm terminals", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const armDir = join(workspaceCwd, ".kraken", "arms", "docs");
    const relativeArmDir = ".kraken/arms/docs";
    const promptsDir = join(process.cwd(), "..", "..", "prompts");
    mkdirSync(armDir, { recursive: true });
    writeFileSync(join(armDir, "CONTEXT.md"), "# Docs\n\nDocumentation team.\n", "utf8");
    ...
    const createResponse = await fetch(`${baseUrl}/api/terminals`, {
      ...
      body: JSON.stringify({ armId: "docs", workspaceMode: "shared" }),
    });
    expect(createResponse.status).toBe(201);
    await expect(createResponse.json()).resolves.toEqual(
      expect.objectContaining({
      ...
      document.terminals.some(
        (terminal) =>
          ...
          terminal.initialInputDraft ===
            `You are working on the Docs section. For tool-list items, context, and docs, check ${relativeArmDir}.`,
      ),
    );
    expect(registryDocument.terminals).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          terminalId: "terminal-1",
          initialInputDraft: `You are working on the Docs section. For tool-list items, context, and docs, check ${relativeArmDir}.`,
        }),
      ]),
    );
  ...
  it("creates isolated worktree terminals with dedicated cwd", async () => {
    ...
    const expectedWorktreePath = join(workspaceCwd, ".kraken", "worktrees", "terminal-1");
    expect(gitClient.getWorktree(expectedWorktreePath)).toEqual(
      expect.objectContaining({
        cwd: workspaceCwd,
        branchName: "kraken/terminal-1",
        baseRef: "HEAD",
      }),
    );
  ...
  it("returns git status for worktree arms", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
    ...
    const worktreePath = join(workspaceCwd, ".kraken", "worktrees", "terminal-1");
    gitClient.setWorktreeStatus(worktreePath, {
      branchName: "kraken/terminal-1",
      upstreamBranchName: "origin/kraken/terminal-1",
      isDirty: true,
      aheadCount: 2,
      behindCount: 1,
      insertedLineCount: 0,
    ...
    const statusResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/status`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("returns 409 for git status on shared arms", async () => {
    const baseUrl = await startServer();
  ...
  it("commits pending worktree changes with a required message", async () => {
    ...
    const commitResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/commit`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  it("pushes worktree branch and updates ahead count", async () => {
    ...
    const pushResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/push`, {
      method: "POST",
      headers: {
        Accept: "application/json",
      },
  ...
  it("syncs worktree branch with base ref", async () => {
    ...
    const syncResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/sync`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  it("returns PR status for worktree arms", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
    ...
    gitClient.setWorktreePullRequest(worktreePath, {
      number: 142,
      url: "https://github.com/kraken-project/kraken/pull/142",
      title: "feat: worktree git lifecycle menu",
      baseRef: "main",
      headRef: "kraken/terminal-1",
      state: "OPEN",
      isDraft: false,
      mergeable: "MERGEABLE",
      mergeStateStatus: "CLEAN",
    ...
    const prStatusResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/pr`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("creates PR for worktree arms and returns PR snapshot", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
    ...
    const createPrResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/pr`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        ...
        body: "Adds PR controls in the arm header.",
        baseRef: "main",
      }),
    });
    ...
    await expect(createPrResponse.json()).resolves.toEqual({
      ...
      status: "open",
      number: 101,
      url: "https://github.com/kraken-project/kraken/pull/101",
      title: "feat: expose worktree lifecycle actions",
      baseRef: "main",
      headRef: "kraken/terminal-1",
      isDraft: false,
  ...
  it("merges the current branch PR for worktree arms", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
    ...
    gitClient.setWorktreePullRequest(worktreePath, {
      number: 190,
      url: "https://github.com/kraken-project/kraken/pull/190",
    ...
    const mergeResponse = await fetch(`${baseUrl}/api/arms/terminal-1/git/pr/merge`, {
      method: "POST",
      headers: {
        Accept: "application/json",
      },
  ...
  it("returns 409 for PR actions on shared arms", async () => {
    const baseUrl = await startServer();
  ...
  it("removes isolated worktree metadata when deleting a worktree arm", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
    ...
    expect(gitClient.hasBranch("kraken/terminal-1")).toBe(false);
  });

  it("returns 409 and keeps arm state when worktree deletion fails", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
    const baseUrl = await startServer({
  ...
  it("refreshes builtin prompts from promptsDir on server start", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    const projectStateDir = mkdtempSync(join(tmpdir(), "kraken-state-test-"));
    const promptsDir = mkdtempSync(join(tmpdir(), "kraken-prompts-test-"));
    ...
    writeFileSync(
      join(projectStateDir, "prompts", "core", "fleet-parent.md"),
      "stale prompt with {{workerBranches}}\n",
      "utf8",
    );
    writeFileSync(
      join(promptsDir, "fleet-parent.md"),
    ...
    const response = await fetch(`${baseUrl}/api/prompts/fleet-parent`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("reads builtin prompts from the live promptsDir after server start", async () => {
    ...
    writeFileSync(join(promptsDir, "arm-update-arm.md"), "version one\n", "utf8");
    ...
    writeFileSync(join(promptsDir, "arm-update-arm.md"), "version two\n", "utf8");

    const response = await fetch(`${baseUrl}/api/prompts/arm-update-arm`, {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
  ...
  it("returns 400 when creating worktree arm outside a git repository", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const gitClient = new FakeGitClient();
  ...
  it("returns 400 when arm name is empty after trimming", async () => {
    const baseUrl = await startServer();
  ...
  it("spawns a shared-workspace todo agent for an individual item", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    mkdirSync(join(workspaceCwd, ".kraken", "arms", "docs-knowledge"), {
      recursive: true,
    });
    writeFileSync(
      join(workspaceCwd, ".kraken", "arms", "docs-knowledge", "CONTEXT.md"),
      "# Docs & Knowledge\n",
      "utf8",
    );
    writeFileSync(
      join(workspaceCwd, ".kraken", "arms", "docs-knowledge", "todo.md"),
    ...
    const solveResponse = await fetch(`${baseUrl}/api/commandDeck/arms/docs-knowledge/todo/solve`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
  ...
  it("auto-renames todo agents from the todo item context on first prompt submit", async () => {
    ...
    const hookResponse = await fetch(
      `${baseUrl}/api/hooks/user-prompt-submit?kraken_session=docs-knowledge-todo-0`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  it("limits fleet prompts to the top-priority items that fit under the child cap", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    mkdirSync(join(workspaceCwd, ".kraken", "arms", "docs-knowledge"), {
      recursive: true,
    ...
    const fleetResponse = await fetch(`${baseUrl}/api/commandDeck/arms/docs-knowledge/fleet`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
    ...
    await expect(fleetResponse.json()).resolves.toEqual({
      armId: "docs-knowledge",
      parentTerminalId: "docs-knowledge-fleet-parent",
      workers: Array.from({ length: MAX_CHILDREN_PER_PARENT }, (_, index) => ({
        terminalId: `docs-knowledge-fleet-${index}`,
        todoIndex: index,
        todoText: `item ${index}`,
      })),
    });
    ...
    const promptTemplate = readFileSync(
      join(process.cwd(), "..", "..", "prompts", "fleet-parent.md"),
      "utf8",
    );
  ...
  it("deletes an arm and removes it from snapshots", async () => {
    const baseUrl = await startServer();
  ...
  it("restores arms across API restarts using persisted registry", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "kraken-api-test-"));
    temporaryDirectories.push(workspaceCwd);
