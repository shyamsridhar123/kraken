...
vi.mock("../src/app/hooks/useCanvasGraphData", () => ({
  useCanvasGraphData: () => ({
    ...
    edges: [],
    armById: new Map(),
    sessionsByArmId: new Map(),
    refresh: vi.fn(),
    refreshCommandDeckArms: vi.fn(),
  }),
}));
...
vi.mock("../src/components/canvas/CanvasArmPanel", () => ({
  CanvasArmPanel: () => null,
}));
  ...
  it("updates an open terminal panel label when the terminal is renamed", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent(
        "panel a:terminal-1 label arm-a",
      );
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId("panel-a:terminal-1")).toHaveTextContent(
        "panel a:terminal-1 label renamed-arm",
      );
    });
  });

  it("shows arm maintenance actions in the context menu and passes the arm ID", async () => {
    const onArmAction = vi.fn().mockResolvedValue(undefined);

    const { container } = render(
      <CanvasPrimaryView columns={[]} isUiStateHydrated onArmAction={onArmAction} />,
    );

    const armNode = container.querySelector('[data-node-id="t:arm-a"]');
    expect(armNode).not.toBeNull();

    fireEvent.contextMenu(armNode as Element, { clientX: 160, clientY: 120 });

    expect(await screen.findByRole("button", { name: "Update To-Do List" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Update Arm" })).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Update To-Do List" }));

    await waitFor(() => {
      expect(onArmAction).toHaveBeenCalledWith("arm-a", "arm-reorganize-todos");
    });
  });
});
