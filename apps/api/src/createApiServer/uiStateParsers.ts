...
export const parseUiStatePatch = (
  ...
  if (record.canvasOpenArmIds !== undefined) {
    if (!Array.isArray(record.canvasOpenArmIds)) {
      return {
        patch: null,
        error: "canvasOpenArmIds must be an array of strings.",
      };
    }

    const canvasOpenArmIds = record.canvasOpenArmIds.filter(
      (id): id is string => typeof id === "string",
    );
    if (canvasOpenArmIds.length !== record.canvasOpenArmIds.length) {
      return {
        patch: null,
        error: "canvasOpenArmIds must be an array of strings.",
      };
    }
    patch.canvasOpenArmIds = canvasOpenArmIds;
  }
