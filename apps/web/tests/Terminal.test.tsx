...
describe("Terminal", () => {
  ...
  it("renders idle badge and updates it from websocket state events", async () => {
    ...
    render(<Terminal terminalId="arm-a" />);
  ...
  it("renders terminal with the provided terminal label", async () => {
    ...
    render(<Terminal terminalId="arm-a-agent-1" terminalLabel="arm-a-agent-1" />);

    expect(screen.getByText("arm-a-agent-1")).toBeInTheDocument();
  });
});
