...
export const GLOBAL_KRAKEN_DIR = join(homedir(), ".kraken");
export const PROJECTS_FILE = join(GLOBAL_KRAKEN_DIR, "projects.json");
export const PROJECT_CONFIG_RELATIVE_PATH = join(".kraken", "project.json");
...
export const ensureGlobalKrakenDir = () => {
  if (!existsSync(GLOBAL_KRAKEN_DIR)) {
    mkdirSync(GLOBAL_KRAKEN_DIR, { recursive: true });
  }
};

export const loadProjectsRegistry = (): ProjectsRegistry => {
  ensureGlobalKrakenDir();
...
export const ensureProjectConfig = (
  ...
  const configPath = resolveProjectConfigPath(workspaceCwd);
  mkdirSync(join(workspaceCwd, ".kraken"), { recursive: true });
...
export const resolveGlobalProjectDir = (projectId: string) =>
  join(GLOBAL_KRAKEN_DIR, "projects", projectId);
...
export const ensureProjectScaffold = (
  ...
  for (const subdirectory of ["arms", "worktrees"]) {
    mkdirSync(join(krakenDir, subdirectory), { recursive: true });
  }

  return ensureProjectConfig(workspaceCwd, preferredName, preferredProjectId);
};

export const hasKrakenGitignoreEntry = (workspaceCwd: string) => {
...
export const ensureKrakenGitignoreEntry = (workspaceCwd: string) => {
  const gitignorePath = join(workspaceCwd, ".gitignore");
  const entry = ".kraken";
...
export const migrateStateToGlobal = (workspaceCwd: string, projectStateDir: string) => {
  const fallbackProjectDir = join(workspaceCwd, ".kraken");
  ...
  const legacyGlobalProjectDir =
    legacyProjectName && currentConfig
      ? join(GLOBAL_KRAKEN_DIR, "projects", legacyProjectName)
      : null;
