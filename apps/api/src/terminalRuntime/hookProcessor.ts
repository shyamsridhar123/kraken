...
export const createHookProcessor = (deps: {
        ...
        SessionStart: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: `curl -s -X POST "${apiBaseUrl}/api/hooks/session-start?kraken_session=$KRAKEN_SESSION_ID" -H 'Content-Type: application/json' -d @- || true`,
                timeout: 5,
              },
            ],
          },
        ...
        UserPromptSubmit: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: `curl -s -X POST "${apiBaseUrl}/api/hooks/user-prompt-submit?kraken_session=$KRAKEN_SESSION_ID" -H 'Content-Type: application/json' -d @- || true`,
                timeout: 5,
              },
            ],
          },
            ...
            hooks: [
              {
                ...
                headers: { "X-Kraken-Session": "$KRAKEN_SESSION_ID" },
                allowedEnvVars: ["KRAKEN_SESSION_ID"],
                timeout: 5,
              },
            ],
          },
        ...
        Stop: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: `curl -s -X POST "${apiBaseUrl}/api/hooks/stop?kraken_session=$KRAKEN_SESSION_ID" -H 'Content-Type: application/json' -d @- || true`,
                timeout: 15,
              },
            ],
          },
  ...
  const handleHook = (
    hookName: string,
    payload: unknown,
    krakenSessionId?: string,
  ): { ok: boolean } => {
    logVerbose(
      `[Hook] Received hook: ${hookName} krakenSession=${krakenSessionId ?? "(none)"}`,
    );
    ...
    if (hookName === "notification") {
      ...
      const session = sessions.get(krakenSessionId);
      if (!session) {
        logVerbose(`[Hook] notification: no session for ${krakenSessionId}, skipping.`);
        return { ok: true };
      }
      ...
      logVerbose(`[Hook] notification: type=${notificationType} session=${krakenSessionId}`);

      if (notificationType === "permission_prompt") {
        session.agentState = "waiting_for_permission";
        session.stateTracker.forceState("waiting_for_permission");
        onStateChange?.(krakenSessionId, "waiting_for_permission", session.lastToolName);
    ...
    if (hookName === "pre-tool-use") {
      ...
      logVerbose(`[Hook] pre-tool-use: tool=${toolName} session=${krakenSessionId}`);
      ...
      if (toolName === "AskUserQuestion") {
        ...
        onStateChange?.(krakenSessionId, "waiting_for_user");
    ...
    if (hookName === "user-prompt-submit") {
      ...
      const terminal = terminals.get(krakenSessionId);
      if (!terminal) {
        return { ok: true };
      }

      // Update last-active timestamp (determines active/inactive on the commandDeck).
      ...
      if (terminal.nameOrigin === "generated") {
        ...
        if (renameContext.length > 0) {
          ...
          if (session) {
            broadcastMessage(session, { type: "rename", armName: derived });
          }
        }
      }
    ...
    if (krakenSessionId && sessions.has(krakenSessionId)) {
      matchedSessionId = krakenSessionId;
      logVerbose(`[Hook] Matched session by kraken_session param: ${matchedSessionId}`);
    } else if (krakenSessionId) {
      logVerbose(
        `[Hook] kraken_session=${krakenSessionId} not found in active sessions, skipping.`,
      );
      return { ok: true };
    } else {
      logVerbose("[Hook] No kraken_session param — ignoring hook from external Claude session.");
      return { ok: true };
    }
