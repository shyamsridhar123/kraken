...
type CreateSessionRuntimeOptions = {
  ...
  getArmWorkspaceCwd: (armId: string) => string;
...
export const createSessionRuntime = ({
  ...
  const resolveSession =
    ...
    ((terminalId: string) => {
      ...
      return {
        ...
        armId: terminal?.armId ?? terminalId,
      };
    });
  ...
  const ensureSession = (sessionId: string, armId: string) => {
    ...
    const armCwd = getArmWorkspaceCwd(armId);
    if (!existsSync(armCwd)) {
      throw new Error(`Terminal working directory does not exist: ${armCwd}`);
    }
    ...
    try {
      pty = spawn(shellLaunch.command, shellLaunch.args, {
        ...
        cwd: armCwd,
        env: createShellEnvironment({ krakenSessionId: sessionId }),
        name: "xterm-256color",
      });
    } catch (error) {
      throw new Error(
    ...
    appendDebugLog(session, `session-start session=${sessionId} arm=${armId}`);
    appendTranscriptEvent(session, sessionId, {
      type: "session_start",
      timestamp: new Date().toISOString(),
    });
  ...
  const handleUpgrade = (request: IncomingMessage, socket: Duplex, head: Buffer): boolean => {
    ...
    const { sessionId, armId } = resolvedSession;

    websocketServer.handleUpgrade(request, socket, head, (websocket: WebSocket) => {
      let session: TerminalSession;
      try {
        session = ensureSession(sessionId, armId);
      } catch (error) {
        sendMessage(websocket, {
          type: "output",
