...
import { CanvasArmPanel } from "./canvas/CanvasArmPanel";
...
type CanvasPrimaryViewProps = {
  ...
  onCanvasOpenArmIdsChange?: (ids: string[]) => void;
  onCanvasTerminalsPanelWidthChange?: (width: number | null) => void;
  onCreateAgent?: (armId: string) => Promise<string | undefined> | undefined;
  onCreateTerminal?: () => Promise<string | undefined> | undefined;
  onCreateWorktreeTerminal?: () => Promise<string | undefined> | undefined;
  onCreateArm?: () => void;
  onSpawnFleet?: (armId: string, workspaceMode: TerminalWorkspaceMode) => Promise<void>;
  onSolveTodoItem?: (armId: string, itemIndex: number) => Promise<void> | void;
  onCaptainAction?: (action: string) => Promise<string | undefined> | undefined;
  onArmAction?: (
    armId: string,
    action: string,
  ...
  onTerminalRenamed?: ((terminalId: string, armName: string) => void) | undefined;
...
export const CanvasPrimaryView = ({
  ...
  const [openArms, setOpenArms] = useState<Map<string, GraphNode>>(new Map());
  ...
  const hasHydratedArms = useRef(false);
  ...
  const resolveActiveSessionNode = useCallback(
    (terminalId: string): GraphNode | null => {
      ...
      const parentNodeId = terminal.parentTerminalId
        ...
        : buildArmNodeId(terminal.armId);
      const anchorNode =
        existingNode?.type === "active-session"
          ? existingNode
          : (nodesById.get(parentNodeId) ??
            nodesById.get(buildArmNodeId(terminal.armId)));
      ...
      return {
        ...
        label: terminal.armName || terminal.label || terminal.terminalId,
  ...
  useEffect(() => {
    setOpenTerminals((current) => {
      ...
      for (const [nodeId, node] of current) {
        ...
        const nextLabel = terminal.armName || terminal.label || terminal.terminalId;
  ...
  // Hydrate open arms from persisted IDs.
  // Gate on arm-type nodes being present (deck API fetch is async).
  const hasArmNodes = simulatedNodes.some((n) => n.type === "arm");
  const openArmCount = openArms.size;
  useEffect(() => {
    if (hasHydratedArms.current) return;
    if (!isUiStateHydrated) return;
    if (!hasArmNodes) return;

    if (canvasOpenArmIds && canvasOpenArmIds.length > 0) {
      const restoredMap = new Map<string, GraphNode>();
      for (const nodeId of canvasOpenArmIds) {
        const node = nodesById.get(nodeId);
        if (node && (node.type === "arm" || node.type === "captain")) {
          restoredMap.set(nodeId, { ...node });
        }
      }
      if (restoredMap.size > 0) {
        setOpenArms(restoredMap);
      }
    }

    hasHydratedArms.current = true;
  }, [isUiStateHydrated, canvasOpenArmIds, hasArmNodes, nodesById]);

  // Persist open arm IDs when they change
  useEffect(() => {
    if (!hasHydratedArms.current) return;
    onCanvasOpenArmIdsChange?.(Array.from(openArms.keys()));
  }, [openArms, onCanvasOpenArmIdsChange]);
  ...
  const handleNodeClick = useCallback(
    (nodeId: string) => {
      ...
      } else if (node.type === "arm" || node.type === "captain") {
        setOpenArms((prev) => {
          const next = new Map(prev);
          if (next.has(nodeId)) {
            next.delete(nodeId);
  ...
  const handleCloseArm = useCallback((nodeId: string) => {
    setOpenArms((prev) => {
      const next = new Map(prev);
      next.delete(nodeId);
      return next;
  ...
  useEffect(() => {
    if (!isHydratingTerminals && openTerminalCount === 0 && openArmCount === 0) return;
  ...
  }, [isHydratingTerminals, openTerminalCount, openArmCount]);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (node.type === "captain") {
        e.preventDefault();
        e.stopPropagation();
        setContextMenu({ kind: "captain", x: e.clientX, y: e.clientY });
        return;
      }
  ...
  const handleCreateAgent = useCallback(
    (armId: string) => {
      ...
      setContextMenu(null);
      const result = onCreateAgent(armId);
  ...
  const handleSpawnFleet = useCallback(
    (armId: string, workspaceMode: TerminalWorkspaceMode) => {
      setContextMenu(null);
      void onSpawnFleet?.(armId, workspaceMode);
    },
    [onSpawnFleet],
  );

  const handleCaptainAction = useCallback(
    (action: string) => {
      setContextMenu(null);
      const result = onCaptainAction?.(action);
      if (result && typeof result.then === "function") {
        void result.then((agentId) => {
          if (agentId) setPendingOpenAgentId(agentId);
        });
  ...
  const handleArmAction = useCallback(
    (armId: string, action: string) => {
      setContextMenu(null);
      const result = onArmAction?.(armId, action);
      if (result && typeof result.then === "function") {
        void result.then((agentId) => {
          if (agentId) setPendingOpenAgentId(agentId);
        });
  ...
  useEffect(() => {
    ...
    if (!openTerminals.has(selectedNodeId) && !openArms.has(selectedNodeId)) {
  ...
  }, [selectedNodeId, openTerminals, openArms]);

  // Separate arm and session nodes for render order
  const armNodes = simulatedNodes.filter(
    (n) => n.type === "arm" || n.type === "captain",
  );
  const sessionNodes = simulatedNodes.filter((n) => {
    if (n.type === "arm" || n.type === "captain") return false;
  ...
  const hasPanels = isHydratingTerminals || openTerminals.size > 0 || openArms.size > 0;
    ...
    <section ref={containerRef} className="canvas-view" aria-label="Canvas graph view">
      <div className={`canvas-graph-panel${hasPanels ? " canvas-graph-panel--split" : ""}`}>
        ...
        >
          ...
          >
            ...
            {/* Render arm nodes (with arms) first */}
            {armNodes.map((node) => {
        ...
        <div className="canvas-toolbar" role="toolbar" aria-label="Canvas actions">
          ...
          <button type="button" className="canvas-toolbar-btn" onClick={onCreateArm}>
            <span className="canvas-toolbar-icon">
              <Hexagon size={14} />
            </span>
            <span className="canvas-toolbar-label">Arm</span>
          </button>
      ...
      {hasPanels && (
        <>
          ...
          >
            {Array.from(openArms.entries()).map(([nodeId, node]) => (
              <CanvasArmPanel
                key={nodeId}
                node={node}
                isFocused={selectedNodeId === nodeId}
                panelRef={setPanelRef(nodeId)}
                arm={armById.get(node.armId) ?? null}
                sessions={sessionsByArmId.get(node.armId) ?? []}
                onClose={() => handleCloseArm(nodeId)}
                onFocus={() => setSelectedNodeId(nodeId)}
                onCreateAgent={(armId) => {
                  handleCreateAgent(armId);
                }}
                onSolveTodoItem={(armId, itemIndex) => {
                  void onSolveTodoItem?.(armId, itemIndex);
                }}
                onSpawnFleet={(armId, workspaceMode) => {
                  handleSpawnFleet(armId, workspaceMode);
                }}
                onNavigateToConversation={onNavigateToConversation}
                onRefreshArmData={refreshDeckArms}
              />
            ))}
      ...
      {contextMenu && (
        <>
          ...
          >
            ...
            {contextMenu.kind === "arm" && (
              <>
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleCreateAgent(contextMenu.armId)}
                >
                  <span className="canvas-context-menu-icon">
                    <TerminalIcon size={14} />
                  </span>
                ...
                <button
                  ...
                  className="canvas-context-menu-item"
                  onClick={() =>
                    handleArmAction(contextMenu.armId, "arm-reorganize-todos")
                  }
                >
                  <span className="canvas-context-menu-icon">
                    <ListTodo size={14} />
                ...
                <button
                  ...
                  className="canvas-context-menu-item"
                  onClick={() =>
                    handleArmAction(contextMenu.armId, "arm-update-arm")
                  }
                >
                  <span className="canvas-context-menu-icon">
                    <Hexagon size={14} />
                ...
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleSpawnFleet(contextMenu.armId, "worktree")}
                >
                  <span className="canvas-context-menu-icon">
                    <Layers size={14} />
                  </span>
                ...
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleSpawnFleet(contextMenu.armId, "shared")}
                >
                  <span className="canvas-context-menu-icon">
                    <Layers size={14} />
                  </span>
            ...
            {contextMenu.kind === "captain" && (
              <>
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleCaptainAction("captain-reorganize-todos")}
                >
                  <span className="canvas-context-menu-icon">
                    <ListTodo size={14} />
                  </span>
                ...
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleCaptainAction("captain-reorganize-arms")}
                >
                  <span className="canvas-context-menu-icon">
                    <Hexagon size={14} />
                  </span>
                ...
                <button
                  type="button"
                  className="canvas-context-menu-item"
                  onClick={() => handleCaptainAction("captain-clean-contexts")}
                >
                  <span className="canvas-context-menu-icon">
                    <Sparkles size={14} />
                  </span>
