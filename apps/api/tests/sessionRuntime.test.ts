...
const createUpgradeRequest = (armId: string) =>
  ({
    url: `/api/terminals/${armId}/ws`,
  }) as IncomingMessage;
...
describe("createSessionRuntime", () => {
  ...
  const createTemporaryDirectory = () => {
    const directory = mkdtempSync(join(tmpdir(), "kraken-session-runtime-test-"));
  ...
  it("keeps a session alive across reconnects and replays scrollback history", () => {
    ...
    const runtime = createSessionRuntime({
      ...
      terminals,
      sessions,
      getArmWorkspaceCwd: () => process.cwd(),
    ...
    expect(
      runtime.handleUpgrade(createUpgradeRequest(armId), {} as Duplex, Buffer.alloc(0)),
    ).toBe(true);

    pty.emitData("first line\r\n");
    pty.emitData("second line\r\n");
    firstSocket.close();
    expect(sessions.has(armId)).toBe(true);
  ...
  it("closes idle sessions after the configured grace timeout", () => {
    ...
    expect(sessions.has(armId)).toBe(false);
  ...
  it("writes normalized transcript events for each terminal session", async () => {
    ...
    const transcriptPath = join(transcriptDirectoryPath, `${encodeURIComponent(armId)}.jsonl`);
  ...
  it("can start a prompted session headlessly and submits the prompt automatically", () => {
    ...
    expect(runtime.startSession(armId)).toBe(true);
    expect(sessions.has(armId)).toBe(true);
  ...
  it("reports runtime state changes through the state-change callback", () => {
    ...
    expect(onStateChange).toHaveBeenCalledWith(armId, "processing", undefined);
