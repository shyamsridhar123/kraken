...
const SKILL_MARKER_START = "<!-- kraken:suggested-skills:start -->";
const SKILL_MARKER_END = "<!-- kraken:suggested-skills:end -->";
...
export const readAvailableClaudeSkills = (workspaceCwd: string): CommandDeckAvailableSkill[] => {
  const roots: Array<{ path: string; source: CommandDeckAvailableSkill["source"] }> = [
    { path: join(workspaceCwd, ".claude", "skills"), source: "project" },
  ];

  const seen = new Map<string, CommandDeckAvailableSkill>();
