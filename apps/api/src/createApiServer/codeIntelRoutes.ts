...
export const handleCodeIntelEventsRoute: ApiRouteHandler = async (
  ...
  if (request.method === "POST") {
    ...
    // Prefer Kraken session ID from header, fall back to Claude Code's own session_id from payload
    const krakenSession =
      typeof request.headers["x-kraken-session"] === "string" &&
      request.headers["x-kraken-session"].length > 0
        ? request.headers["x-kraken-session"]
        : undefined;
    const claudeSession =
      payload && typeof payload.session_id === "string" && payload.session_id.length > 0
        ? payload.session_id
        : undefined;
    const sessionId = krakenSession ?? claudeSession ?? "unknown";
