...
const inferTerminalNameOrigin = (terminalId: string, armName: string): TerminalNameOrigin => {
  if (armName === terminalId || /^Kraken Terminal \d+$/.test(armName)) {
    return "generated";
  }
...
const parsePersistedUiState = (value: unknown): PersistedUiState => {
  ...
  if (Array.isArray(value.canvasOpenArmIds)) {
    nextState.canvasOpenArmIds = value.canvasOpenArmIds.filter(
      (id): id is string => typeof id === "string",
    );
  }
 ...
 * Each old arm entry becomes a terminal where terminalId = armId.
...
const migrateV2ToV3 = (
  ...
  const rawArms = record.arms;
  if (!Array.isArray(rawArms)) {
    throw new Error(`Invalid registry arms array (${registryPath}).`);
  }

  const terminals = new Map<string, PersistedTerminal>();
  for (const item of rawArms) {
    if (item === null || typeof item !== "object") {
      throw new Error(`Invalid arm entry in registry (${registryPath}).`);
    }

    const entry = item as Record<string, unknown>;
    const armId =
      typeof entry.armId === "string"
        ? entry.armId
        : null;
    const armName =
      typeof entry.armName === "string"
        ? entry.armName
        : null;
    const createdAt = typeof entry.createdAt === "string" ? entry.createdAt : null;

    if (!armId || !armName || !createdAt) {
      throw new Error(`Incomplete arm entry in registry (${registryPath}).`);
    }

    const rawWorkspaceMode = entry.workspaceMode;
    const workspaceMode: ArmWorkspaceMode =
      rawWorkspaceMode === "worktree" || rawWorkspaceMode === "shared"
        ? rawWorkspaceMode
        : "shared";

    if (terminals.has(armId)) {
      throw new Error(`Duplicate arm id in registry (${registryPath}): ${armId}`);
    }
    ...
    terminals.set(armId, {
      ...
      armId,
      armName,
      nameOrigin: inferTerminalNameOrigin(armId, armName),
      createdAt,
      workspaceMode,
    });
  }
...
const parseV3Terminals = (
  ...
  for (const item of rawTerminals) {
    ...
    const armName = typeof entry.armName === "string" ? entry.armName : null;
    const createdAt = typeof entry.createdAt === "string" ? entry.createdAt : null;

    if (!terminalId || !armId || !armName || !createdAt) {
    ...
    const terminal: PersistedTerminal = {
      ...
      nameOrigin: isTerminalNameOrigin(entry.nameOrigin)
        ? entry.nameOrigin
        : inferTerminalNameOrigin(terminalId, armName),
      createdAt,
      workspaceMode,
    };
