...
const resolvePackageRoot = () => {
  const envRoot = process.env.KRAKEN_PACKAGE_ROOT?.trim();
  if (envRoot) {
...
const initializeProject = (workspaceCwd: string, preferredName?: string) => {
  const projectName = preferredName?.trim() || basename(workspaceCwd) || "kraken-project";
...
const resolveStartupProjectContext = (workspaceCwd: string) => {
  ...
  const projectDisplayName = basename(workspaceCwd) || "kraken-project";
...
const initProject = (name?: string) => {
  ...
  console.log(
    `${created ? "Initialized" : "Updated"} Kraken project "${projectConfig.displayName}" at ${projectPath}`,
  );
  console.log("  .kraken/ directory ready (project metadata, arms, worktrees)");
  console.log(`  Global state: ${projectStateDir}`);
  console.log("  .gitignore updated");
  console.log("\nRun `kraken` to start the dashboard.");
};
...
const resolveRuntimeApiBase = () => {
  const explicitBase =
    process.env.KRAKEN_API_ORIGIN?.trim() || process.env.KRAKEN_API_BASE?.trim();
  if (explicitBase) {
    return explicitBase;
  }
...
const apiError = () => {
  console.error(
    `Error: Could not reach API at ${resolveRuntimeApiBase()}. Start Kraken in this project first.`,
  );
  process.exit(1);
};

const maybeOpenBrowser = (url: string) => {
  if (process.env.KRAKEN_NO_OPEN === "1" || process.env.CI === "1") {
    return;
  }
...
const startServer = async () => {
  ...
  const apiServer = createApiServer({
    ...
    allowRemoteAccess: process.env.KRAKEN_ALLOW_REMOTE_ACCESS === "1",
  });
...
const armCreate = async () => {
  const name = args[2];
  if (!name || name.startsWith("-")) {
    console.error("Error: arm name is required.");
    process.exit(1);
  }
  ...
  try {
    const response = await fetch(`${apiBase}/api/command-deck/arms`, {
      method: "POST",
    ...
    console.log(`Created arm "${data.armId}"`);
  } catch {
    apiError();
  }
};
...
const armList = async () => {
  ...
  try {
    const response = await fetch(`${apiBase}/api/command-deck/arms`);
    if (!response.ok) {
      console.error("Error: failed to fetch arms.");
      process.exit(1);
    }

    const arms = (await response.json()) as Array<Record<string, unknown>>;
    if (arms.length === 0) {
    ...
    for (const arm of arms) {
      const description = arm.description ? ` — ${arm.description}` : "";
      console.log(`  ${arm.armId}${description}`);
    }
  } catch {
    apiError();
  }
...
const channelSend = async () => {
  ...
  const fromTerminalId = parseFlag("--from") ?? process.env.KRAKEN_SESSION_ID ?? "";
...
const main = async () => {
  ...
  if (command === "projects" || command === "project") {
    ...
    if (projects.length === 0) {
      console.log(
        "No projects registered yet. Run `kraken` or `kraken init` in a project directory.",
      );
      return;
    }
  ...
  console.log(`Usage:
  kraken                             Start the dashboard in the current project
  kraken init [project-name]         Initialize the current directory explicitly
  kraken projects                    List registered projects

  kraken arm create <name>           Create an arm (Kraken must be running)
  kraken arm list                    List arms
  kraken terminal create [options]   Create a terminal
  ...
  kraken channel send <id> <msg>     Send a channel message
  kraken channel list <id>           List channel messages`);
  process.exit(1);
};
