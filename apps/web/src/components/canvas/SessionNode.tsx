...
export const SessionNode = ({ node, isSelected, onPointerDown, onClick }: SessionNodeProps) => {
  ...
  const isFleetWorker = !!node.parentTerminalId;
  ...
  return (
    ...
    >
      ...
      {/* Fleet worker ring — double concentric circles */}
      {isFleetWorker && (
        <>
          <circle
            className="canvas-node-ring canvas-node-ring--fleet"
            r={node.radius + 5}
            fill="none"
            stroke={color}
          />
          <circle
            className="canvas-node-ring canvas-node-ring--fleet-outer"
            r={node.radius + 9}
            fill="none"
            stroke={color}
          />
