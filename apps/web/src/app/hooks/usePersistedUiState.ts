...
const DEFAULT_CANVAS_OPEN_ARM_IDS: string[] = [];
...
const areUiStateSnapshotsEqual = (
  ...
  areStringArraysEqual(left.canvasOpenArmIds, right.canvasOpenArmIds) &&
...
type UsePersistedUiStateResult = {
  ...
  canvasOpenArmIds: string[];
  setCanvasOpenArmIds: Dispatch<SetStateAction<string[]>>;
  canvasTerminalsPanelWidth: number | null;
...
export const usePersistedUiState = ({
  ...
  const [canvasOpenArmIds, setCanvasOpenArmIds] = useState<string[]>(
    DEFAULT_CANVAS_OPEN_ARM_IDS,
  );
  ...
  const applyHydratedUiState = useCallback(
    (snapshot: FrontendUiStateSnapshot | null, nextColumns: TerminalView) => {
      ...
      const activeArmIds = new Set(nextColumns.map((entry) => entry.armId));
      ...
      if (!snapshot) {
        lastPersistedUiStateRef.current = buildPersistedUiStateSnapshot({
          ...
          canvasOpenArmIds: DEFAULT_CANVAS_OPEN_ARM_IDS,
      ...
      const nextCanvasOpenTerminalIds = snapshot.canvasOpenTerminalIds
        ...
        : DEFAULT_CANVAS_OPEN_TERMINAL_IDS;
      const nextCanvasOpenArmIds = snapshot.canvasOpenArmIds
        ? retainActiveTerminalIds(snapshot.canvasOpenArmIds, activeArmIds)
        : DEFAULT_CANVAS_OPEN_ARM_IDS;
      ...
      lastPersistedUiStateRef.current = buildPersistedUiStateSnapshot({
        ...
        canvasOpenArmIds: nextCanvasOpenArmIds,
      ...
      if (snapshot.canvasOpenArmIds) {
        setCanvasOpenArmIds(nextCanvasOpenArmIds);
      }
  ...
  useEffect(() => {
    ...
    const activeArmIds = new Set(columns.map((entry) => entry.armId));
    ...
    setCanvasOpenArmIds((current) => retainActiveTerminalIds(current, activeArmIds));
  }, [columns]);
