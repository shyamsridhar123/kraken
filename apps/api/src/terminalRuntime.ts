...
import type { TerminalSnapshot } from "@kraken/core";
...
export type { ChannelMessage, ArmGitStatusSnapshot, ArmPullRequestSnapshot } from "@kraken/core";
...
export const createTerminalRuntime = ({
  ...
  getApiBaseUrl = () => process.env.KRAKEN_API_ORIGIN ?? "http://127.0.0.1:8787",
}: CreateTerminalRuntimeOptions) => {
  ...
  const registryPath = join(stateDir, "state", "arms.json");
  ...
  const isDebugPtyLogsEnabled = process.env.KRAKEN_DEBUG_PTY_LOGS === "1";
  const ptyLogDir = process.env.KRAKEN_DEBUG_PTY_LOG_DIR ?? join(stateDir, "logs");
  ...
  const resolveTerminalSession = (
    terminalId: string,
  ): { sessionId: string; armId: string } | null => {
    const terminal = terminals.get(terminalId);
    if (terminal) {
      return {
        sessionId: terminalId,
        armId: terminal.worktreeId ?? terminal.armId,
      };
    }
  ...
  const sessionRuntime = createSessionRuntime({
    ...
    getArmWorkspaceCwd: worktreeManager.getArmWorkspaceCwd,
  ...
  const allocateTerminalId = () => {
    ...
    while (candidateNumber < Number.MAX_SAFE_INTEGER) {
      ...
      if (worktreeManager.hasArmWorktree(candidateId)) {
        candidateNumber += 1;
        continue;
      }
  ...
  const allocateDefaultTerminalName = (): string => {
    ...
    const pattern = /^Kraken Terminal (\d+)$/;
    for (const t of terminals.values()) {
      const match = pattern.exec(t.armName);
      if (match) usedNumbers.add(Number(match[1]));
    }
    let n = 1;
    while (usedNumbers.has(n)) n++;
    return `Kraken Terminal ${n}`;
  };
  ...
  const createTerminal = ({
    ...
    const terminal: PersistedTerminal = {
      ...
      armName: effectiveName,
      nameOrigin: nameOrigin ?? (armName ? "user" : "generated"),
    ...
    const effectiveWorktreeId = worktreeId ?? armId;
    const shouldCreateWorktree = workspaceMode === "worktree";
    if (shouldCreateWorktree) {
      worktreeManager.createArmWorktree(effectiveWorktreeId, baseRef);
    }
    ...
    if (terminal.agentProvider === "claude-code") {
      ...
      try {
        const hookTargetCwd = shouldCreateWorktree
          ? worktreeManager.getArmWorkspaceCwd(effectiveWorktreeId)
          : workspaceCwd;
  ...
  return {
    ...
    patchUiState(patch: PersistedUiState): PersistedUiState {
      ...
      if (patch.canvasOpenArmIds !== undefined) {
        uiState.canvasOpenArmIds = [...patch.canvasOpenArmIds];
      }
    ...
    renameTerminal(terminalId: string, armName: string): TerminalSnapshot | null {
      const terminal = terminals.get(terminalId);
      if (!terminal) {
        return null;
      }

      terminal.armName = armName;
    ...
    deleteTerminal(terminalId: string): boolean {
      ...
      for (const cascadeTerminalId of cascadeTerminalIds) {
        ...
        if (cascadeTerminal.workspaceMode === "worktree") {
          worktreeManager.removeArmWorktree(
            cascadeTerminal.worktreeId ?? cascadeTerminal.armId,
          );
        }
